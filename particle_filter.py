...
class Particle_filter():
    ...
    def get_gaussian(self, value, mean):

        """Function that returns the normal pdf"""

        denom = math.sqrt(2*math.pi*self.variance)
        val = ((value - mean) ** 2)/(-2 * self.variance)
    ...
    def get_estimate(self, particles, reading):
        ...
        for particle in particle:
            err0 = self.get_gaussian(particle[0], reading[0])
            err1 = self.get_gaussian(particle[2], reading[2])
