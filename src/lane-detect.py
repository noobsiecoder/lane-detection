...
PATHS = ["./data/drive_0001_sync/image_02/data/", "./data/drive_0002_sync/image_02/data/", "./data/drive_0011_sync/image_02/data/", "./data/drive_0015_sync/image_02/data/", "./data/drive_0027_sync/image_02/data/", "./data/drive_0029_sync/image_02/data/", "./data/drive_0032_sync/image_02/data/", "./data/drive_0038_sync/image_02/data/", "./data/drive_0042_sync/image_02/data/"]
...
def planar_warp(img):
    ...
    old_points = np.float32([[100, 10], [300, 10], [1, img.shape[0]], [450, img.shape[0]]])
...
def hough_lines(o_img, img, matrix):
    ...
    inv_matrix = np.linalg.inv(matrix)
    count = 0
    lines = cv.HoughLinesP(img,1, np.pi / 180, 20, minLineLength=100,maxLineGap=90)
    ...
    if lines is not None:
        for line in lines:
            ...
            de_warped_lines.append([p1_de_warped[0], p1_de_warped[1], p2_de_warped[0], p2_de_warped[1]])

    # Draw the de-warped lines on the original image
    for line in de_warped_lines:
        x1, y1, x2, y2 = line
        # print(x1[0], y1)
        cv.line(o_img, (x1[0], y1[0]), (x2[0], y2[0]), (0, 0, 255), 6)  # Example: Draw a line
...
if __name__ == "__main__":
    ...
    image_files = sorted([os.path.join(path, file) for file in os.listdir(path) if file.endswith(('.png', '.jpg'))])
    # print(image_files)
    ...
    output_video = cv.VideoWriter('output_video_0029.avi', fourcc, 10.37, (width, height))
